...
@Path(FeedsServiceKafka.PATH)
public interface FeedsServiceKafka {
	...
	@Path("/personal/{" + USER + "}")
	@Produces(MediaType.APPLICATION_JSON)
	List<Message> pull_getTimeFilteredPersonalFeed(@PathParam(USER) String user, @QueryParam(TIME) long time, @HeaderParam(HEADER_VERSION) Long version);
}
